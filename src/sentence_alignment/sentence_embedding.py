...
def encode_sentences(edition ,lang, model):
    ...
    if not os.path.exists(Path(embed_data_path / edition)): # if data/embed/edition doesn't exist
        os.makedirs(Path(embed_data_path / edition)) # make it 

    output_path = Path(embed_data_path / edition / '{}.emb'.format(lang)) # the path to output
    input_path = Path(token_data_path / edition / '{}.txt'.format(lang)) # the path to tokenised sentences

    print("\n\n{}\n\n".format(output_path))

    command = f'bash {laser_path}/tasks/embed/embed.sh ' # the command without params
    command += "{} {} {}".format(input_path, output_path, model) # add params which are source text, output path & lang model
